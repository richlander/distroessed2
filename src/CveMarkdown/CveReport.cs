...
public static class CveReport
{
    ...
    public static void WriteCveTable(CveSet cves, StreamWriter writer)
    {
        ...
        {
            cveTable.AddRow($"[{cve.Id}][{cve.Id}]", cve.Description, cve.Product, Join(cve.Platforms), cve?.Cvss ?? "");
        }
    ...
    public static void WritePackageTable(CveSet cves, StreamWriter writer)
    {
        ...
        foreach (Cve cve in cves.Cves)
        {
            ...
            {
                packageTable.AddRow($"[{cve.Id}][{cve.Id}]", Report.MakePackageString(package.Name), $">={package.MinVulnerableVersion}", $"<={package.MaxVulnerableVersion}", package.FixedVersion);
            }
        }
    ...
    public static void WriteCommitTable(CveSet cves, StreamWriter writer)
    {
        ...
        {
            commitTable.AddRow($"[{commit.Cve}][{commit.Cve}]", Report.MakeLinkFromBestSource(commit, commit.Branch, cves.Source.BranchUrl, null), Report.MakeLinkFromBestSource(commit, commit.Hash, cves.Source.CommitUrl, commit.Url));
        }
